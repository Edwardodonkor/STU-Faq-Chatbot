...
async function handleSendMessage() {
    ...
    try {
        ...
        const response = await fetch('/chat', {
            ...
            body: JSON.stringify({
                message: message,
                userId: getOrCreateUserId() // Get or create a unique user ID
            }),
        });
        ...
        if (audioUrl) {
            playAudio(audioUrl); // Play audio response
        }
...
function handleRecognitionResult(event) {
    try {
        ...
        if (confidence > 0.5 || confidence === undefined) {
            ...
            // Immediately send the message after recognition
            handleSendMessage();
        } else {
...
function handleRecognitionEnd() {
    ...
    setTimeout(() => {
        if (!isProcessing) { // Only reset if not currently processing a sent message
            resetVoiceButton();
        }
    }, 500);
}
...
window.addEventListener('beforeunload', function() {
    ...
    stopAudioPlayback(); // Stop audio on page unload
});
...
document.addEventListener('visibilitychange', function() {
    ...
    if (document.hidden) {
        stopAudioPlayback(); // Stop audio if page becomes hidden
    }
});
